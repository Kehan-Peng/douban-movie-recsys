...
这是一个本地单机运行的电影推荐项目：前台继续使用 `Flask + SQLite`，后台本地强化学习重排、语义增强推荐、实验可视化和更规范的 `repository / service / schema` 分层。
...
- LLM打标签：Qwen 模型给电影数据打标签，限制于15个电影类别。
...
- 后台可视化：管理台页面和 `api/v1/admin/*` 接口，实验页支持折线图/柱状图查看算法趋势。
...
这次落地的是“本地轻量 `Word2Vec-style / GloVe-style` 实现”，目标是兼顾算法和单机可运行，不引入重型外部 embedding 训练框架。

这样做的价值：

- 不是一上来就上重型分布式训练，而是在当前数据规模和项目目标下，先落地可解释、可跑通、可对比的语义 embedding。
- 算法深度比 `TF-IDF` 更强：共现建模、局部上下文、词向量空间、语义近邻。
...
不走“全量 PPO 主推荐器”，而是走更适合单机小数据场景的路线：
...
- PPO 不是主召回器，而是本地 rerank 层，解决小数据场景下“直接全量 PPO 收益不稳、复杂度过高”的问题。
- 在线反馈不是逐条训练，而是写入 `rl_experience` 经验池，达到阈值后再批量更新，避免本机资源抖动。
- 模型版本保存在本地文件系统，版本元数据登记到 `model_registry`，可以在后台页或 CLI 中快速回滚。
- 行为、模型、实验相关的数据库访问已经从业务模块中抽到 `repository/service`，降低 `myutils/query.py` 耦合。
...
- 数据采集、缓存、训练、评估、版本回滚和后台管理。
- 不依赖独立服务器，单机即可完成采集、训练、推荐和可视化展示。
- 在当前规模下选择“混合召回 + PPO 重排”而不是盲目全量 PPO。
- 这里落地的是本地轻量 `Word2Vec-style / GloVe-style`，而不是直接接重型 embedding 训练框架。
- 线上行为 -> 经验池 -> 批量训练 -> 新版本上线 -> 指标观察 -> 回滚闭环。
- baseline -> semantic embedding -> PPO rerank -> 实验页趋势验证的渐进优化。
